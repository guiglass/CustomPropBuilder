    ...
    public class Interactable : MonoBehaviour
    {
        ...
        protected virtual void CreateHighlightRenderers()
        {
            ...
            for (int skinnedIndex = 0; skinnedIndex < existingSkinnedRenderers.Length; skinnedIndex++)
            {
				...
				newSkinned.gameObject.layer = LayerMask.NameToLayer ("UI"); //So it wont show in mirrors
				newSkinned.updateWhenOffscreen = false;
				newSkinned.lightProbeUsage = UnityEngine.Rendering.LightProbeUsage.Off;
				newSkinned.reflectionProbeUsage = UnityEngine.Rendering.ReflectionProbeUsage.Off;
				newSkinned.receiveShadows = false;
				newSkinned.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
            ...
            for (int filterIndex = 0; filterIndex < existingFilters.Length; filterIndex++)
            {
				...
				newRenderer.gameObject.layer = LayerMask.NameToLayer ("UI"); //So it wont show in mirrors
		...
		public event UnityAction<Hand> onHandHoverBegin;
		public event UnityAction<Hand> onHandHoverEnd;
