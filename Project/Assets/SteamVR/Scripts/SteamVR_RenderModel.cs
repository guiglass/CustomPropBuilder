    ...
    public class SteamVR_RenderModel : MonoBehaviour
    {
        ...
        public void UpdateComponents(CVRRenderModels renderModels)
        {
            ...
            for (int childIndex = 0; childIndex < transform.childCount; childIndex++)
            {
				...
				*/
				//Added boolean check for custom controller visibility options
				bool visible = (
					componentState.uProperties & (uint)EVRComponentProperty.IsVisible
				) != 0;

				if (visible != child.gameObject.activeSelf && child.GetComponent<MeshRenderer>() != null)
				{
					child.gameObject.SetActive(visible);
				}
            }
