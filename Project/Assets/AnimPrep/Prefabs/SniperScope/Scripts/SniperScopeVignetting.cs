...
namespace VR_SniperScope
{
	...
	public class SniperScopeVignetting : MonoBehaviour
	{
		...
		public void Start() {				
			...
			m_ChromAberrationMaterial = new Material(chromAberrationShader);
		}
		...
		void OnRenderImage (RenderTexture source, RenderTexture destination)
		{
			...
			if (doPrepass)
			{
				...
				m_VignetteMaterial.SetVector("_Center", -center);	

				m_VignetteMaterial.SetFloat ("_Intensity", intensity * 10);//(1.0f / (1.0f - intensity) - 1.0f));		// intensity for vignette
				m_VignetteMaterial.SetFloat("_Blur", blur); //(1.0f / (1.0f - blur)) - 1.0f);					// blur intensity
			...
			Graphics.Blit (doPrepass ? color : source, texture, m_ChromAberrationMaterial, mode == AberrationMode.Advanced ? 2 : 1);
