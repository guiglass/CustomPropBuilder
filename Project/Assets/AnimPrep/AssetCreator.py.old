...
#This is the raw text for the blenderscript.py script that is run from within the user's .blend file from subprocess launches the their scene file. Dont forget this file becomes a temporary file and is saved into a temp directory.
...
class Interface:
	...
	def LogMessage(self, message, tag=None): #tag may be ['error','warning','notice'] or None
		if self.start_time is -1:
			...
			self.log.insert( '{:1.8f}'.format(time.time()- self.start_time) + " - " +  message, tag)
